...
static char needs_fixing = 0;

static u32 fixed_u32(u32 n) {
    if(needs_fixing) {
        return ntohl(n);
    }
    return  n;
...
static void fix_info(struct cramfs_info *info) {
        if(needs_fixing) {
                info->crc = ntohl(info->crc);
                info->edition = ntohl(info->edition);
                info->blocks = ntohl(info->blocks);
                info->files = ntohl(info->files);
        }
}
...
static void fix_inode(struct cramfs_inode *inode) {
        if(needs_fixing) {
            ...
            inode->uid = ntohs(inode->uid);
            inode->size = ntohl(inode->size);
            inode->offset = ntohl(inode->offset);
        }
}
...
static void fix_super(struct cramfs_super *super) {
    if(super->magic != CRAMFS_MAGIC) {
        needs_fixing = 1;
        super->magic = ntohl(super->magic);
        super->size = ntohl(super->size);
        super->flags = ntohl(super->flags);
        super->future = ntohl(super->future);
...
static void test_crc(int start)
{
        ...
        if (buf != MAP_FAILED) {
                ((struct cramfs_super *) (buf+start))->fsid.crc = (u32)crc32(0L, Z_NULL, 0);
                crc = (u32)crc32(crc, buf+start, super.size-start);
                ...
                for (;;) {
                        ...
                        if (length == 0) {
                                ((struct cramfs_super *) buf)->fsid.crc = (u32)crc32(0L, Z_NULL, 0);
                        }
                        length += retval;
                        if (length > (super.size-start)) {
                                crc = (u32)crc32(crc, buf, (u32)(retval - (length - (super.size-start))));
                                break;
                        }
                        crc = (u32)crc32(crc, buf, retval);
                }
                free(buf);
        }

        if (crc != fixed_u32(super.fsid.crc)) {
                die(FSCK_UNCORRECTED, 0, "crc error.  Expected %08x got %08x", fixed_u32(super.fsid.crc), crc);
        }
}
...
{
        unsigned int block = (unsigned int)(offset >> ROMBUFFER_BITS);
...
static struct cramfs_inode *read_super(void)
{
        ...
        {
                die(FSCK_UNCORRECTED, 0, "bad root offset (%lu) - expecting %lu or %lu", offset, sizeof(struct cramfs_super), PAD_SIZE + sizeof(struct cramfs_super));
        }
...
static void do_uncompress(char *path, int fd, unsigned long offset, unsigned long size)
{
        ...
        do {
                ...
                if (curr == next) {
                        ...
                        out = uncompress_block(romfs_read(curr), (int)(next - curr));
                }
                if (size >= PAGE_CACHE_SIZE) {
...
static void do_directory(char *path, struct cramfs_inode *i)
{
        ...
        while (count > 0) {
                struct cramfs_inode *child = iget((unsigned int)offset);
                int size;
...
static void do_symlink(char *path, struct cramfs_inode *i)
{
        ...
        size = uncompress_block(romfs_read(curr), (int)(next - curr));
        if (size != i->size) {
